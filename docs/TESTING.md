...
The repository contains 360+ MoonBit test blocks plus a data-driven scenario
corpus:

- `testdata/scenarios/`: 290 rule-scenario fixtures (282 generated by
  `devtools/scenario_gen`, 8 hand-written). A CLI-level test replays every
  fixture and enforces the final-acceptance floor of at least 250 passing
  scenarios.
- `testdata/compat/`: differential compatibility fixtures matched against
  documented reference behavior (see `REFERENCES.md`).
- `moon bench` blocks: policy decision, variant keys, MemoryStore lookup, and
  full runtime lifecycle (see `BENCHMARKS.md`).
